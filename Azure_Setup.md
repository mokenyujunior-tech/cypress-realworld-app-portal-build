...
- Region: Canada Central
- Linux Plan: Created new with name `plan-payflow`, and pricing plan Premium V3 P0v3. Chose it becuse it supports deployment slots. Standard is legacy
...
**Step 4:** Enabled System Assigned Managed Identity on the Production Web App. Then copied and saved the production slot's Object (principal) ID.
...
**Step 1:** Downloaded both Publish Profiles, one for each slot.
...
**Step 3:** Created a deployment file with the three-job pipeline. Check [deploy](.github/workflows/deploy.yml)
Also I deleted all original Cypress workflow files from `.github/workflows/`. They were test-only pipelines that weren't helpful and created failures on every push.
