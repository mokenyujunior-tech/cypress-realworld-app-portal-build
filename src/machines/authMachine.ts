  ...
  },
  {
    services: {
      performSignup: async (ctx, event) => {
        ...
        const resp = await httpClient.post(`${apiBaseUrl}/users`, payload);
        history.push("/signin");
        return resp.data;
      },
      performLogin: async (ctx, event) => {
        return await httpClient
          .post(`${apiBaseUrl}/login`, event)
          .then(({ data }) => {
            history.push("/");
            return data;
          })
      ...
      getUserProfile: async (ctx, event) => {
        const resp = await httpClient.get(`${apiBaseUrl}/checkAuth`);
        return resp.data;
      },
      ...
      updateProfile: async (ctx, event: any) => {
        const payload = omit("type", event);
        const resp = await httpClient.patch(`${apiBaseUrl}/users/${payload.id}`, payload);
        return resp.data;
      },
      performLogout: async (ctx, event) => {
        localStorage.removeItem("authState");
        return await httpClient.post(`${apiBaseUrl}/logout`);
      },
